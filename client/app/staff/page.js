...
export default function StaffPage() {
    ...
    const [addForm, setAddForm] = useState({ name: "", email: "", password: "", role: "KITCHEN" });
    const [teamFilter, setTeamFilter] = useState("KITCHEN"); // STAFF | DELIVERY
    ...
    const handleAddStaff = async (e) => {
        ...
        try {
            ...
            setAddForm({ name: "", email: "", password: "", role: "KITCHEN" });
            fetchAllStaff();
        } catch (err) {
                                        ...
                                        >
                                            ...
                                            <span className="col-span-2">
                                                <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-200/80 bg-amber-50 px-3 py-1 text-[11px] font-extrabold uppercase tracking-wide text-amber-800">
                                                    ...
                                                    {member.role === "KITCHEN" ? "👨‍🍳 Kitchen" : member.role === "DELIVERY" ? "🛵 Delivery" : member.role}
                                                </span>
                                            </span>
